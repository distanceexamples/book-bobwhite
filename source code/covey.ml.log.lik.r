...
typ<-c(0,p[5])      # coefficient for Type level "TREAT" (level "CONTROL" is absorbed in the intercept)
day<-p[6]           # coefficient for JDctr
st<-c(0,p[7:9])   # state coefficients for levels "MS","NC","TN" (level "MO" is absorbed in the intercept)
std.ran<-exp(p[10]) # the random effect standard deviation on the log-scale

# Part 2 of covey.ml.log.lik(): the likelihood component pertaining to the detection model
